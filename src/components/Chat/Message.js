                ...
                <ScrollToBottom style={{height:'70vh'}}>
              {
                (messages && messages.length>0)?
                    messages.map((msg,index) => {
                        return (
                    <Alert className="row"  style={{float:msg.user === 'admin'?'none':username === msg.user?'left':'right',margin:'0.5rem'}} variant={msg.user === 'admin'?'dark':username === msg.user?"danger":"primary"} >
                        <div>
