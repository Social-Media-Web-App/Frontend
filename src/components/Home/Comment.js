...
const Comment = ({isAuthenticated,comments}) => {
    ...
    return (
            <div>
               <Alert style={{marginBottom:'0',cursor:'pointer'}} onClick={() => setShowComment(!showComment)} variant={showComment?"danger":"success"}>
                   ...
                   comments.map((comment,index) => {
                       return (
                        <Alert key={index} variant="info" style={{marginBottom:'0',marginTop:'0.3rem',marginLeft:'0.1rem',marginRight:'0.1rem'}}>
                            ...
                            <Image className="bg-primary" rounded style={{width:'35px',height:'35px'}} src={comment.userid.avatar?comment.userid.avatar:userImg} />&nbsp;&nbsp;
